  ...
  return (
    <nav className={`fixed inset-x-0 top-0 z-50 h-16 flex items-center justify-between px-5 md:px-8 transition-all duration-300 ${solid ? "bg-white/96 backdrop-blur-xl border-b" : ""}`}
      ...
      <div className="flex items-center gap-6">
        ...
        <button onClick={onCart} className="relative w-10 h-10 rounded-xl flex items-center justify-center transition-colors"
          style={{ background: T.surface }} aria-label="Open cart">
          <ShoppingCart size={18} style={{ color: T.mid }} />
          {cartCount > 0 && (
            <span className="absolute -top-1 -right-1 w-[18px] h-[18px] rounded-full text-white text-[10px] font-extrabold flex items-center justify-center animate-pop"
